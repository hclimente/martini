...
set.seed(0)
causal <- sample(V(net), 5)
eff <- rnorm(5)

test_that("simulate_phenotype runs", {
  expect_type(simulate_phenotype(gwas, causal, h2 = 1, effectSize = eff), "double")
  expect_type(simulate_phenotype(gwas, causal, h2 = 1, effectSize = eff,
                                 qualitative = T, ncases = 3000, ncontrols = 3000), "double")
})

# get 20 random snps and 20 random effect sizes
causal <- sample(V(net), 20)
eff <- rnorm(length(causal))
X <- as(gwas$genotypes, "numeric")
X_causal <- X[, gwas$map$snp.names %in% names(causal)]

# association between causal SNPs and the qualitative phenotype
Y_ql <- simulate_phenotype(gwas, causal, h2 = 1, 
                           effectSize = eff, 
...
Y_qt <- simulate_phenotype(gwas, causal, h2 = 1, effectSize = eff)
